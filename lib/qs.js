...
export const { encode, decode } = (() => {
    let regexp = /=(.+?)(&|#|$)/g,
        encodeFind = (str, $1, $2) => '=' + escape($1) + $2,
        decodeFind = (str, $1, $2) => '=' + unescape($1) + $2;

    // 返回函数
    return {
        encode(queryString) {
            return queryString ? queryString.replace(regexp, encodeFind) : '';
        },
        decode(queryString) {
            return queryString ? queryString.replace(regexp, decodeFind) : '';
        }
    };
})();
